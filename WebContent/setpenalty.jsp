...
<div class="fh5co-pricing-style-2" style="background:rgba(0, 0, 0, 0.0);margin-top:10px;">
  ...
  query="select * from productdetail where email='"+e+"' and batch='"+b+"'";
  }
  if(err.equals("removepenalty")){
  query="select * from productdetail where email='"+e+"' and batch='"+b+"' and penalty > 0";  
  }
  ...
  while(rs.next()){
  count++;%>
	   <form action="penaltyquantity.jsp" >

	  
		            <div class="pricing-item wow fadeInUp" data-wow-duration="1s" data-wow-delay="1.1s" style="margin-right:50px;width:400px;">
		                 <input type="hidden" value="<%=err %>" name="penaltytype" />
		                 <input type="hidden" value="<%=rs.getString(2).toString() %>" name="penaltyemail" />
		                 <input type="hidden" value="<%=rs.getString(7)%>" name="penaltyname" />
		                 <h4 class="pricing-title" style="padding-top:10px;"><%= rs.getString(7) %></h4>	                
		                 <input class="btn btn-success btn-outline" type="submit" style="margin-top:15px;" value="Add Penalty">
		            </div>
            </form>   
  <%}
  if(count==0){%>
	  <div style="height:400px;width:1000px;"> <h1 style="text-align:center;color:white;margin:40px;">No Items for removing penalty !!!</h1></div>
<%
  }
  %>  
  </div></div></div></div>
